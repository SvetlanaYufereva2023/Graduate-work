...
Процедура СформироватьДвижения(); 
	...
	ДвиженияНДФЛ = Движения.ВКМ_Удержания.Записывать = Истина;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ДвиженияНДФЛ.Сотрудник = ВыборкаДетальныеЗаписи.Сотрудник; 
		ДвиженияНДФЛ.ВидРасчета = ПланыВидовРасчета.ВКМ_Удержания.НДФЛ;
		ДвиженияНДФЛ.БазовыйПериодНачало = НачалоМесяца(Дата);
		ДвиженияНДФЛ.БазовыйПериодКонец = КонецМесяца(Дата);   
		ДвиженияНДФЛ.ПериодРегистрации = Дата;
...
Процедура РассчитатьНДФЛ();  
	...
	Запрос.Текст = 
		...
		|	СУММА(ЕСТЬNULL(ВКМ_УдержанияБазаВКМ_ОсновныеНачисления.РезультатБаза, 0)) КАК Результат
		|ИЗ
